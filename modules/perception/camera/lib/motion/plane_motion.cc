...
void PlaneMotion::update_motion_buffer(const base::VehicleStatus &vehicledata,
  ...
  ADEBUG<<"update_motion_buffer SIZE"<<mot_buffer_->size();
  ...
  ADEBUG<<"time_difference"<<time_difference_;
...
void PlaneMotion::add_new_motion(double pre_image_timestamp,
  ...
  ADEBUG<<"IN ADD NEW MOTION RAW_QUEUE_SIZE"<<raw_motion_queue_.size();
  ADEBUG<<"vehicledata->time_ts "<<vehicledata->time_ts<<"raw_motion_queue_.back().time_ts"<<raw_motion_queue_.back().time_ts;
  ...
  ADEBUG<<"IN ADD NEW MOTION2  flag"<<motion_operation_flag;
  if (motion_operation_flag != RESET) {
    ADEBUG<<"GENERATE MATRIX1";
    generate_motion_matrix(vehicledata);
    ADEBUG<<"GENERATE MATRIX2";
