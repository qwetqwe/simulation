...
DEFINE_string(params_dir, "/apollo/modules/perception/data/params",
              "params directory");
DEFINE_string(visualize_dir, "/apollo/data/vis", "visualize directory");
DEFINE_double(camera_fps, 15, "camera_fps");
DEFINE_bool(do_undistortion, false, "do_undistortion");
DEFINE_string(undistortion_save_dir, "/apollo/data/vis/undistortion_result",
              "Directory to save undistored images.");
DEFINE_string(save_dir, "/apollo/data/vis/result",
...
int work() {
  ...
  while (fin >> line) {
    ...
    Eigen::Affine3d c2n;
    if (!transform_server.QueryTransform(camera_name, "velodyne128", &c2n)) {
