...
bool FusionCameraDetectionComponent::Init() {
  output_obstacles_channel_name_="perception/obstacles";
  writer_ =
...
void FusionCameraDetectionComponent::OnReceiveImage(
  ...
  std::shared_ptr<apollo::perception::PerceptionObstacles> test_message(
      new apollo::perception::PerceptionObstacles);
  test_message->add_perception_obstacle()->set_id(1);
  test_message->mutable_perception_obstacle(0)->set_height(2);
  test_message->set_error_code(error_code);
      writer_->Write(test_message);
      AINFO<<"out message size　"<<test_message->perception_obstacle_size()<<" height"<<test_message->perception_obstacle(0).height();
...
int FusionCameraDetectionComponent::InitCameraListeners() {
    ...
    std::function<void(const ImageMsgType &)> camera_callback1 =
        std::bind(&FusionCameraDetectionComponent::OnReceiveImage, this,
                  std::placeholders::_1, "front_12mm");
    auto camera_reader1 = node_->CreateReader("/apollo/sensor/camera/traffic/image_long", camera_callback1);
    typedef std::shared_ptr<apollo::drivers::Image> ImageMsgType;
    std::function<void(const ImageMsgType &)> camera_callback2 =
        std::bind(&FusionCameraDetectionComponent::OnReceiveImage, this,
                  std::placeholders::_1,  "front_6mm");
    auto camera_reader2 = node_->CreateReader("/apollo/sensor/camera/traffic/image_short", camera_callback2);
  return cyber::SUCC;
}
