...
class OfflineLidarObstaclePerception {
 public:
  ...
  bool setup() {
    //FLAGS_config_manager_path = "modules/perception/production/";
  ...
  bool run() {
    ...
    for (size_t i = 0; i < pcd_file_names.size(); ++i) {
      ...
      std::string file_name = GetFileName(pcd_file_names[i]);
      auto iterr=file_name.erase(file_name.end()-4,file_name.end());
      ...
      if (pose_folder != "") {
        ...
        if (!apollo::cyber::common::PathExists(pose_file_name)) {
          pose_file_name = pose_folder + "/" + file_name + ".pose";
        }
        int idt = 0;
      ...
      if (FLAGS_enable_tracking) {
        ...
        if (FLAGS_use_tracking_info) {
          ...
          AINFO<<"DEBUG 1 size"<<objects.size();
          for (std::size_t j = 0; j < objects.size(); ++j) {
            AINFO<<"OBJ"<<objects[j]->id<<"res obj"<<result_objects[j]->id;
            CHECK(objects[j]->id == result_objects[j]->id);
            auto tmpp=frame_->lidar2world_pose.inverse();
