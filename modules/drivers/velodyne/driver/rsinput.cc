 ...
 * @param private_nh: ROS private handle for calling node.
 **************************************************************/
InputSocket::InputSocket(apollo::drivers::velodyne::Config config) : Input(config)
{
  if (this->dev_ip_.empty())
  {
 ...
 * @return error code
 *****************************************************/
E_INPUT_STATE InputSocket::getPacket(rslidarPacket* pkt, const unsigned int timeout)
{
  E_INPUT_STATE ret;
