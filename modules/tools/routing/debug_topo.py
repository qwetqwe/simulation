...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="A tool to debug topo")
    parser.add_argument(
        "routing_map_dir", action="store", type=str, help="routing map  only bin support also need base_map.bin in the same dir")
    #map_dir = util.get_map_dir(sys.argv)
    g_args = parser.parse_args()
    graph = util.get_topodata(g_args.routing_map_dir)
    base_map = util.get_mapdata(g_args.routing_map_dir)
