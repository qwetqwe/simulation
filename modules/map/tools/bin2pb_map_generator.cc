...
DEFINE_string(map_name, "base_map", "input or output map name");
int main(int argc, char **argv) {
  ...
  const auto map_filename = FLAGS_map_dir + "/"+FLAGS_map_name+".bin";
  apollo::hdmap::Map pb_map;
  CHECK(apollo::cyber::common::GetProtoFromBinaryFile(map_filename,&pb_map));
  const std::string output_pb_file = FLAGS_output_dir + "/"+FLAGS_map_name+".txt";
