...
DEFINE_string(base_map_path,"","base_map dir path,must xml or txt");
DEFINE_string(routing_map_path,"","routing map path");
int main(int argc, char **argv) {
  ...
  apollo::routing::GraphCreator creator(FLAGS_base_map_path, FLAGS_routing_map_path, routing_conf);
  CHECK(creator.Create()) << "Create routing topo failed!";

  AINFO << "Create routing topo successfully from " << FLAGS_base_map_path << " to "
        << FLAGS_routing_map_path;
  return 0;
}
